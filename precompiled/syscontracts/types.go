...
var (
	...
	NFTContractAddress           = common.HexToAddress("0x0000000000000000000000000000000000000012") // The PlatONE Precompiled contract addr for NFT contract
...
type NodeInfo struct {
	Name  string `json:"name,omitempty,required"` //全网唯一，不能重复。所有接口均以此为主键。
	Owner string `json:"owner"`
	Desc  string `json:"desc"`
	Type  uint32 `json:"type"` // 2:观察者节点；1:共识节点
	// status 1为正常节点, 2为删除节点
...
type NodeQueryInfo struct {
	Name     string `json:"name,omitempty,required"` //全网唯一，不能重复。所有接口均以此为主键。
	Status   uint32 `json:"status,required"`         // status 1为正常节点, 2为删除节点
	Type     uint32 `json:"type"`                    // 2:观察者节点；1:共识节点
	DelayNum uint64 `json:"delayNum,omitempty"`      //共识节点延迟设置的区块高度 (可选, 默认实时设置)
}
