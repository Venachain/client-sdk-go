...
func (firewallClient FireWallClient) FwStatus(txparam common_sdk.TxParams) (string, error) {
	funcName := "__sys_FwStatus"
	funcParams := []string{firewallClient.ContractAddress}
	result, err := firewallClient.contractCallWrap(txparam, funcParams, funcName, precompile.FirewallManagementAddress)
	if err != nil {
		return "", err
	}
...
func (firewallClient FireWallClient) FwStart(txparam common_sdk.TxParams) (string, error) {
...
func (firewallClient FireWallClient) FwClose(txparam common_sdk.TxParams) (string, error) {
...
func (firewallClient FireWallClient) FwExport(txparam common_sdk.TxParams, filePath string) (bool, error) {
...
func (firewallClient FireWallClient) FwImport(txparam common_sdk.TxParams, filePath string) (string, error) {
...
func (firewallClient FireWallClient) FwNew(txparam common_sdk.TxParams, action, targetAddr, api string) (string, error) {
	funcName := "__sys_FwAdd"
	return firewallClient.fwCommon(txparam, action, targetAddr, api, funcName)
}

// 删除一条防火墙规则
func (firewallClient FireWallClient) FwDelete(txparam common_sdk.TxParams, action, targetAddr, api string) (string, error) {
	funcName := "__sys_FwDel"
	return firewallClient.fwCommon(txparam, action, targetAddr, api, funcName)
}

// 重置防火墙规则
func (firewallClient FireWallClient) FwReset(txparam common_sdk.TxParams, action, targetAddr, api string) (string, error) {
	funcName := "__sys_FwSet"
	return firewallClient.fwCommon(txparam, action, targetAddr, api, funcName)
}

func (firewallClient FireWallClient) fwCommon(txparam common_sdk.TxParams, action, targetAddr, api, funcName string) (string, error) {
...
// 如果action 为""，默认清除所有规则
func (firewallClient FireWallClient) FwClear(txparam common_sdk.TxParams, action string) (string, error) {
	if action == "" {
		result1, err := firewallClient.fwClearCommon(txparam, "reject")
		result2, err := firewallClient.fwClearCommon(txparam, "accept")
		if err != nil {
			return "", err
		} else {
	...
	result, err := firewallClient.fwClearCommon(txparam, action)
	if err != nil {
		return "", err
	}
	return result, nil
...
func (firewallClient FireWallClient) fwClearCommon(txparam common_sdk.TxParams, action string) (string, error) {
